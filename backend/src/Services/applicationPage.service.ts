...
import { sendEmail, getApplicationOpenedEmailTemplate } from '../utils/emailHelper';
...
export class ApplicationPageService {
    ...
    private async sendNotificationsToWaitingUsers() {
        try {
            const waitingUsers = await this.notificationService.getUnnotifiedRequests();
            ...
            for (const user of waitingUsers) {
                try {
                    ...
                    const emailHtml = getApplicationOpenedEmailTemplate(user.fullName, language);
                    
                    const subject = language === 'tr' ? 'Başvurular Açıldı! 🎉' :
                                  language === 'de' ? 'Bewerbungen Sind Jetzt Geöffnet! 🎉' :
                                  language === 'ru' ? 'Заявки Теперь Открыты! 🎉' :
